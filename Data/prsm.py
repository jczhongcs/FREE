...
import Data.prot_seq as ProtSeq
...
class Prsm:
  ...
  def annotate(self, env_list):
    nterm_masses, cterm_masses = ProtSeq.gene_theo_ions(self.prot_sequence, self.acetylation)  #可以通prsm得到的蛋白质序列算出N和C端离子质量，即可得到14种离子的理论质量
    Proton = 1.00727647
    H = 1.007825035
    O = 15.99491463
    CO = 12.0000 + O
    ...
    # b -ion
    self.add_annotation(env_list, nterm_masses, 0.0, "B")    #先得到toppic计算出的同位素峰值质量，以及头文件中得到的解卷积得到的单同位素峰质量,当精度等于15*碎片质量/1000000时且env的头文件的离子类型为空时，则添加离子类型
    # y -ion
