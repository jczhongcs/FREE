...
def Hdf5_generator(X, y, batch_size, nsamples):
  ...
  while True:
    if start_idx + batch_size > nsamples:
      start_idx = 0
    x_batch = X[start_idx:start_idx+batch_size, ...][:,:,0:8]
...
def split_data_single(data_files, labels):
  train_data_files = data_files[0:int(0.8*len(data_files))] #使用了80%做训练文件
  train_labels = labels[0:int(0.8*len(labels))]             #同时使用了80%的标签文件
  validation_data_files = data_files[int(0.8*len(data_files)):] #使用了剩下的20%做验证文件
...
def shuffle_data(labels_dict):
  ...
  c = list(zip(data_files, labels))#将对象中对应的元素打包成一个个元组,然后返回由这些元组组成的列表。
  shuffle(c)                       #把数组中的元素按随机顺序重新排列。
  data_files, labels = zip(*c)     #*将元祖解压为列表
...
def print_training_history(history, ouput_dir):
  ...
  f.writelines("\nTraining_Accuracy :" + str(history.history['accuracy']) + " \nValidation_Accuracy :" + str(history.history['val_accuracy']) ) # Write a string to a file
