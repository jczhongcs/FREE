...
from Data.env_header import EnvHeader
...
def _read_header(env_lines):
  ...
  for i in range(len(env_lines)):
    ...
    if("INTE_DOT_PROC" in line and "BIN_IDX" not in line):
      inte_dot_proc = float(mono[1][0:len(mono[1])-1])
    if("INTE_OS_DIS" in line and "BIN_IDX" not in line):
      inte_os_dis = float(mono[1][0:len(mono[1])-1])
    if("INTE_MHD_DIS" in line and "BIN_IDX" not in line):
      inte_mhd_dis = float(mono[1][0:len(mono[1])-1])
    if("INTE_QBXF_DIS" in line and "BIN_IDX" not in line):
      inte_qbxf_dis = float(mono[1][0:len(mono[1])-1])
    if("INTE_NOM_OS_DIS" in line and "BIN_IDX" not in line):
      inte_nom_os_dis = float(mono[1][0:len(mono[1])-1])

  header = EnvHeader.get_header_with_anno(spec_id, base_inte, peak_id,
      charge, mono_mz, mono_mass, inte_sum, topfd_score, ion_type, label, pred_score,inte_dot_proc,inte_os_dis,inte_mhd_dis,inte_qbxf_dis,inte_nom_os_dis)
...
def _read_feature(env_lines):
  exp_line = "BIN_IDX NORM_THEO_INTE NORM_EXP_INTE ENV_CHAR MASS_DIFF INTE_DIFF LOG_THEO_INTE LOG_BASE_INTE INTE_DOT_PROC INTE_OS_DIS INTE_MHD_DIS INTE_QBXF_DIS INTE_NOM_OS_DIS" #在其后添加新特征
  ...
  while(env_lines[i].strip() != end_line): 
    ...
    inte_dot_proc=float(mono[8])
    inte_os_dis=float(mono[9])
    inte_mhd_dis=float(mono[10])
    inte_qbxf_dis=float(mono[11])
    inte_nom_os_dis=float(mono[12])
    feature = Feature(bin_index, norm_theo_peak_inte, norm_exp_peak_inte, charge, mass_diff, inte_diff, log_theo_intensity, log_base_intenisty,inte_dot_proc,inte_os_dis,inte_mhd_dis,inte_qbxf_dis,inte_nom_os_dis)
