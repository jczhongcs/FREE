...
def convert(in_path, out_path, no_tests=False):
    ...
    model = keras.models.load_model(in_path, custom_objects={'Attention': Attention})
...
class CustomGlorotUniform(Initializer):
    ...
    def __call__(self, shape, dtype=None):
        ...
        limit = np.sqrt(6 / (fan_in + fan_out)) * self.scale
...
def main():
    ...
    #in_path = sys.argv[1]
    #out_path = sys.argv[2]
    in_path='E:\\SXR\\model_attention_10.h5'
    #custom_objects = {'glorot_uniform': glorot_uniform}
    #model = tf.keras.models.load_model('E:\\SXR\\model_attention_10.h5', custom_objects={'Attention': Attention})
    #model.add(Dense(64, kernel_initializer='glorot_normal', activation='relu'))
    #model.add(Dense(64, kernel_initializer=CustomGlorotUniform(), activation='relu'))
    out_path ='E:\\SXR\\my_model.json'
    no_tests = False
    class Attention(layers.Layer):
        def __init__(self, name=None, **kwargs):
            super(Attention, self).__init__(name=name, **kwargs)
